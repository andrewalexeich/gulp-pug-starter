...
* чтобы начать работу, введите команду: ```yarn run dev``` (режим разработки). Удалите заглушку в файле ```src/views/index.pug```, а также стили к ней в теге ```head``` в файле ```src/views/layouts/default.pug```
